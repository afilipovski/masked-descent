...
@export var attack_recoil_time: float = 0.5
...
func _handle_attack_recoil(_delta: float):
	## Handle attack recoil - skeleton stops and steps back slightly, returns true if occurred
	...
	if player:
		...
		velocity = retreat_direction * speed * 0.4
		return true
